...
latest_mead_text <- format(round(tail(mead_records,1)$elevation), big.mark = ",")
...
year_change_mead_elevation_text <- 
  if (round(year_change_mead_elevation) > 0) { 
    paste("up ",format(round(year_change_mead_elevation),big.mark = ","),"", sep = "")
  } else if (round(year_change_mead_elevation) == 0) {
    paste("unchanged", sep = "")
  } else { 
    paste("down ",format(round(-1*year_change_mead_elevation),big.mark = ","),"", sep = "")
  }
...
ten_year_change_mead_elevation_text <- 
  if (round(ten_year_change_mead_elevation) > 0) { 
    paste("up ",format(round(ten_year_change_mead_elevation), big.mark = ","),"", sep = "")
  } else if (round(ten_year_change_mead_elevation) == 0) {
    paste("unchanged", sep = "")
  } else { 
    paste("down ",format(round(-1*ten_year_change_mead_elevation), big.mark = ","),"", sep = "")
  }
...
latest_powell_text <- format(round(tail(powell_records,1)$elevation), big.mark = ",")
...
year_change_powell_elevation_text <- 
  if (round(year_change_powell_elevation) > 0) { 
    paste("up ",format(round(year_change_powell_elevation),big.mark = ","),"", sep = "")
  } else if (round(year_change_powell_elevation) == 0) {
    paste("unchanged", sep = "")
  } else { 
    paste("down ",format(round(-1*year_change_powell_elevation),big.mark = ","),"", sep = "")
  }
...
ten_year_change_powell_elevation <- elevation <- latest_powell - ten_year_ago_powell
ten_year_change_powell_elevation_text <- 
  if (round(ten_year_change_powell_elevation) > 0) { 
    paste("up ",format(round(ten_year_change_powell_elevation),big.mark = ","),"", sep = "")
  } else if (round(ten_year_change_powell_elevation) == 0) {
    paste("unchanged", sep = "")
  } else { 
    paste("down ",format(round(-1*ten_year_change_powell_elevation),big.mark = ","),"", sep = "")
  }
...
## mead ----
fig <- highchart() |> 
  hc_add_series(mead_records, "line", hcaes(x = date,
                                            y = elevation),
                color = "#199fa8",
                yAxis = 0,
                animation = FALSE,
...
## powell ----
fig <- highchart() |> 
  hc_add_series(powell_records, "line", hcaes(x = date,
                                              y = elevation),
                color = "#199fa8",
                yAxis = 0,
                animation = FALSE,
...
- ",year_change_mead_elevation_text," feet from a year ago
- ",ten_year_change_mead_elevation_text," feet from ten years ago
...
- ",year_change_powell_elevation_text," feet from a year ago
- ",ten_year_change_powell_elevation_text," feet from ten years ago

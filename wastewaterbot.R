...
iwss <- iwss_download %>%
  ...
  mutate(rsv_avg = zoo::rollmean(rsv,
                                 ...
                                 align = "right"))
iwss_longer <- iwss |> 
  select(sars_cov_2,influenza_a,influenza_b,rsv,Date,
         sars_cov_2_avg,influenza_a_avg,influenza_b_avg,rsv_avg) |> 
  pivot_longer(!Date) |> 
  filter(Date > today(tzone = "America/Chicago")-years(1) ) |> 
  mutate(contains_avg = str_detect(name, "avg")) |> 
  mutate(contains_avg = if_else(contains_avg,
                                "avg",
                                "not_avg")) |> 
...
all_colors <- c(
  ...
  "#B45F06","blue","purple","black"
)

p <- ggplot() +
  geom_line(data = iwss_longer |> filter(contains_avg == "not_avg"),
            aes(x = as.Date(Date),
                y = value,
                group = name),
            color = "lightgray",

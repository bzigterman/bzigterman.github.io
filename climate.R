...
GISTEMP <- fread(input = url, na.strings = "***") |>
  janitor::clean_names()
...
past_year <- monthly |>
  mutate(
    j_d = round(rollmean(value, k = 12, fill = NA, align = "right"), digits = 2)
  ) |>
  drop_na() |>
  tail(1) |>
  select(year, j_d)
...
fig <- hchart(
  ...
  type = "column",
  hcaes(x = date, y = j_d),
  tooltip = list(valueSuffix = "°C", valueDecimals = 2, xDateFormat = "%Y"),
  name = "Anomaly",
  states = list(
    inactive = list(
      enabled = FALSE
  ...
  hc_yAxis(
    ...
    plotLines = list(
      list(
        label = list(text = "Paris Agreement Goal", align = "left"),
        color = "#999999",
        #width = 1.5,
        value = 2,
        zIndex = 1
      ...
      list(
        label = list(text = "Paris Agreement Stretch Goal", align = "left"),
        color = "#999999",
        #width = 1.5,
        value = 1.5,
        zIndex = 1
...
saveWidget(
  widget = fig,
  file = "interactive/climate_temp_anomaly.html",
  selfcontained = FALSE,
  libdir = "interactive"
)
...
<iframe src=\"https://cdn.climatechangetracker.org/embedding/yearly-average-temperature-anomaly?tempunit=c\" scrolling=\"no\" frameBorder=\"0\" style=\"width:100%; height:400px\"></iframe>

<iframe src=\"https://cdn.climatechangetracker.org/embedding/human-induced-and-observed-global-warming?tempunit=c\" scrolling=\"no\" frameBorder=\"0\" style=\"width:100%; height:400px\"></iframe>

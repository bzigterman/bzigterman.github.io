...
om_hourly <- as_tibble(om$hourly) |>
  ...
  mutate(
    windGust = if_else(windgusts_10m - windSpeed > 10, windgusts_10m, NA)
  ) |>
  ...
  mutate(
    apparent_temperature = case_when(
...
if (data_check) {
  webcams <- webcams_json$reportTables$cells[[1]] |>
    unnest(cols = c(imageDirections), names_repair = "universal") |>
    janitor::clean_names() |>
...
# text ----
now <- as_datetime(now())
now_formatted <- strftime(x = now, tz = "US/Central", format = "%I:%M% %p")

text <- head(paste0(
  "Champaign weather at ",
  now_formatted,
  ":

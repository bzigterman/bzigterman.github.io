...
western_plot <- ggplot(
  western_standings,
  aes(x = reorder(team_label, win_pct), y = win_pct)
) +
  geom_rect(xmin = 9.5, xmax = Inf, ymin = 0, ymax = Inf, fill = "grey85") +
  geom_rect(xmin = 5.5, xmax = 9.5, ymin = 0, ymax = Inf, fill = "grey95") +
  geom_hline(yintercept = 0.5, color = "grey50", size = .2) +
  geom_col(aes(fill = win_pct), width = 1) +
  scale_fill_gradient(guide = NULL, low = "#fd8d3c", high = "#800026") +
  ...
  theme_minimal() +
  labs(x = NULL, y = NULL, title = "Western") +
  theme(
    legend.title = element_blank(),
    panel.grid.major.y = element_line(colour = "grey93"),
    plot.title = element_text(hjust = 1),
    plot.background = element_rect(fill = "white", color = "white"),
    plot.margin = margin(t = 5, r = 5, b = 0),
...
eastern_plot <- ggplot(
  ...
  geom_rect(xmin = -Inf, xmax = 6.5, ymin = 0, ymax = Inf, fill = "grey85") +
  geom_rect(xmin = 6.5, xmax = 10.5, ymin = 0, ymax = Inf, fill = "grey95") +
  geom_hline(yintercept = 0.5, color = "grey50", size = .2) +
  geom_col(aes(fill = win_pct), width = 1) +
  scale_fill_continuous(guide = NULL, low = "#3690c0", high = "#023858") +
  coord_cartesian(ylim = c(nba_min, nba_max)) +
  geom_text(
    aes(label = team_label),
    family = "mono",
  ...
  theme_minimal() +
  labs(x = NULL, y = NULL, title = "Eastern") +
  theme(
    legend.title = element_blank(),
    plot.background = element_rect(fill = "white", color = "white"),
    plot.margin = margin(t = 0, b = 0, l = 0),
    panel.grid = element_blank(),
    panel.grid.major.y = element_line(colour = "grey93"),
...
eastern_plot
p <- plot_grid(western_plot, eastern_plot, align = "h", rel_widths = c(6, 7))
p

file <- tempfile(fileext = ".png")
ggsave(file, plot = p, device = "png", width = 4, height = 2.25, dpi = 320)
...
west_leaders <- western_standings %>%
  ...
  mutate(
    text = if_else(
      ...
      paste0(team, ": ", wins, "–", losses, ", ", conference_games_behind, "\n")
    )
  ) |>
  select(text) %>%
  slice_head(n = 11)

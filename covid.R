...
saveWidget(
  widget = fig,
  file = "interactive/champaign_covid.html",
  selfcontained = FALSE,
  libdir = "interactive"
)
...
cdc_usa <- cdc_json$us_trend_by_Geography_v2 |>
  ...
  select(Date, COVID_deaths_weekly, percent_pos)
...
fig <- hchart(
  cdc_usa,
  type = "line",
  hcaes(x = Date, y = COVID_deaths_weekly),
  states = list(
    inactive = list(
      enabled = FALSE
    )
  ...
  hc_add_series(
    ...
    tooltip = list(valueDecimals = 0, valueSuffix = "{value}%"),
...
saveWidget(
  widget = fig,
  file = "interactive/usa_covid.html",
  selfcontained = FALSE,
  libdir = "interactive"
)
...
fig <- hchart(
  owid_new_cases_deaths_world,
  type = "line",
  hcaes(x = Date, y = avg_new_cases),
  states = list(
    inactive = list(
      enabled = FALSE
    )
...
saveWidget(
  widget = fig,
  file = "interactive/world_covid.html",
  selfcontained = FALSE,
  libdir = "interactive"
)
